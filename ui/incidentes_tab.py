...
class IncidentesTab:
    ...
    def agregar_incidente(self):
        ...
        if not (desc and gravedad and tipo_val and estado_val and usuario_val):
        ...
        cursor = conn.cursor()
        cursor.execute("""
            INSERT INTO Incidente(id_tipo, id_estado, id_usuario, descripcion, fecha, gravedad)
            VALUES (?, ?, ?, ?, ?, ?)
        """, (id_tipo, id_estado, id_usuario, desc, fecha, gravedad))
        conn.commit()
        conn.close()
    ...
    def buscar_id_por_valor(self, tabla, valor):
        conn = sqlite3.connect(DB_PATH)
        cursor = conn.cursor()
        if tabla == "TipoIncidente":
            cursor.execute("SELECT id_tipo FROM TipoIncidente WHERE nombre_tipo = ?", (valor,))
        elif tabla == "Estado":
            cursor.execute("SELECT id_estado FROM Estado WHERE nombre_estado = ?", (valor,))
        elif tabla == "Usuario":
            cursor.execute("SELECT id_usuario FROM Usuario WHERE nombre = ?", (valor,))
        row = cursor.fetchone()
        conn.close()
        return f"{row[0]} - {valor}" if row else ""
    ...
    def actualizar_incidente(self):
        ...
        cursor.execute("""
            UPDATE Incidente
            SET id_tipo = ?, id_estado = ?, id_usuario = ?, descripcion = ?, gravedad = ?
            WHERE id_incidente = ?
        """, (id_tipo, id_estado, id_usuario, desc, gravedad, self.id_incidente_actual))
        conn.commit()
        conn.close()
